...
import { Upload, Play, Pause, AlertCircle, Loader2, Scan } from 'lucide-react';
...
const VideoMonitor: React.FC<VideoMonitorProps> = ({ 
  ...
  useEffect(() => {
    if (seekTo !== null && seekTo !== undefined && videoRef.current) {
        // Only seek if the difference is significant to avoid stutter loops
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        try {
          // Single Pipeline: Video Analysis
  ...
  return (
    <div className="relative h-full w-full bg-stone-900 rounded-3xl overflow-hidden shadow-md group border border-stone-800">
        ...
        {videoSrc ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-stone-500 bg-[#1c1c1e]">
            <div 
              ...
              className="cursor-pointer flex flex-col items-center hover:text-stone-300 transition group/upload"
            >
      ...
      {/* Playback Controls */}
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex justify-between items-end opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-50">
        <div className="flex space-x-3">
          <button 
            onClick={togglePlay}
            className="w-10 h-10 flex items-center justify-center bg-white/10 backdrop-blur-md rounded-full hover:bg-white/20 text-white border border-white/10 transition hover:scale-105 active:scale-95"
          >
